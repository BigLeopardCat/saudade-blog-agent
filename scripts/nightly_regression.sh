#!/usr/bin/env bash
# Nightly regression: 92 gate assertions + fallback replay + golden set (13 real-LLM cases)
...
fail=0
echo "--- test_gates (92 断言) ---" >> "$LOG"
$PY test_gates.py >> "$LOG" 2>&1 || { fail=1; echo "[$TS] test_gates FAILED" >> "$LOG"; }
echo "--- test_fallback_replay ---" >> "$LOG"
$PY test_fallback_replay.py >> "$LOG" 2>&1 || { fail=1; echo "[$TS] test_fallback_replay FAILED" >> "$LOG"; }
