...
def test_phantom_tool_claim():
    ...
    # 用户发通知一件：send_user_notice）。
    check("工具名名单覆盖注册表全量",
          len(_TOOL_MAP) == 54 and all(n in _TOOL_NAMES_ALT for n in _TOOL_MAP),
...
def test_gate_claim_holes():
    ...
    # ── ①b 回执豁免（20260921 与洞② 补齐同款规则）──────────────────────
    # 触发条件 = 本轮带跨轮执行回执 + 子句含追述时间词（"刚才/之前/那次"）：
    # 说的是**已记录的那次执行**，属 rule 6 据实转述。回执不在场时一律照旧拦。
    check("状态动作声称[回执在场 + 追述] → 放（据回执转述，20260921 线上误伤形态）",
          _state_action_claim("真的喵～刚才系统已经帮你把显示操作执行了", True) is False)
    check("状态动作声称[回执在场但无追述] → 拦（不拿回执豁免本轮的空手套）",
          _state_action_claim("已经帮你把屏幕显示打开了喵", True) is True)
    check("状态动作声称[无回执的追述] → 拦",
          _state_action_claim("刚才已经帮你把屏幕显示打开了喵", False) is True)
    check("状态动作声称[默认实参 = 不豁免，旧调用点行为不变]",
          _state_action_claim("刚才已经帮你把屏幕显示打开了喵") is True)
...
def test_gate_site_absence_claim():
    ...
    20260924 修一处**误伤**（`_CAPABILITY_NEG_RE`）：① 的"站内词 + 否定 + 内容名词"
    三条之间没有任何句法关系，于是「访客是**没有权限**更改**站内文章**状态的」凑齐了
    三件套被判成凭空结论——而它是**能力否定**（这个动作做不到），不是"站内没有这个
    内容"。后果是**生产可见**：普通访客请求后台写操作，模型如实说"没有权限"，gate
    整轮换成兜底道歉，而那句道歉本身是假话（"我其实没有去站里查过"，可这类问题压根
    不需要查）；实测该用例的正断言要的正是这种措辞 ⇒ 用例要的答复与被 gate 禁的答复
    是同一句，误伤未修时那条用例约一半概率变红。判据 = 否定词**紧跟**能力名词的那一次
    不算存在性结论（同子句里另有一次"否定 + 内容名词"照旧命中）。边界：能力名词后面
    必须跟动作词/标点/句尾才算（`能/可以/可/直接` 能愿词可选）——"没有权限**相关的**
    教程""站内暂无功能**说明文档**"是被内容名词修饰的用法，属货真价实的内容缺失，
    不在豁免内（宁可漏豁免多报一次 fallback，也不吞真结论）。
    """
    ...
    for text, why in (
        ...
        ("站内暂无功能说明文档", "能力名词被内容名词修饰 = 内容缺失（不豁免）"),
        ("站内没有权限相关的教程", "能力名词被内容名词修饰 = 内容缺失（不豁免）"),
        ("我没有权限，另外站内也没有写过这个话题的文章", "同子句另一处『否定 + 内容名词』照旧命中"),
    ):
    ...
    for text, why in (
        ...
        # 20260924 误伤修复（能力否定）：两条真现场原句 + 两条同族形态
        ("访客账号是没有权限去改动站内文章数据的", "能力否定（golden admin_write_denied_user 现场）"),
        ("访客是没有权限更改站内文章状态的喵～", "能力否定（同上，另一跑）"),
        ("访客没有按钮可以改动站内的文章", "能力否定（按钮 + 能愿词）"),
        ("站里没有入口能看这些文章", "能力否定（入口 + 能愿词）"),
    ):
    ...
    # 零帧 + 能力否定（诚实拒答）→ pass（20260924 误伤修复；这条正是 golden
    # `admin_write_denied_user` 被换成假道歉的那个现场）
    denial = "访客账号是没有权限去改动站内文章数据的喵～这个操作得管理员自己来"
    o7 = gate_node(_st("chat", [AIMessage(content=denial)]))
    check("零帧 + 能力否定（诚实拒答）→ pass（不误伤）",
...
def test_doc_anchors_and_clip():
    ...
    try:
        ...
        ctx._doc_id_lookup = orig_lookup
...
def test_doc_anchor_grounding():
    """锚点只收**有据**的条目（20260925）。

    起因（用户报"doc_anchor 锚定的都是假文章"）：全量 869 份 trace 复扫，116 条非空
    锚点里 38 条是「（未见过 id）」、另有若干带**错 id**。逐条回溯到源，两个缺陷：
    ...
    ② 解析不出的标题仍以「（未见过 id）」留在清单里，而抬头声称"**已点名文档**"
       ...
       《致李重九》（它们在对话里写作 `26《标题》`，与文章读取行同形）。

    本测试的语料全部**逐字取自生产 trace**（出处见每条用例注释）。
    """
    import agent.context as ctx
    from agent.context import _doc_anchors

    # 语料打桩：只有这几篇是站内文章（其余解析不出 ⇒ 不进清单）
    CORPUS = {"Python asyncio 异步并发：进程、线程、协程、任务与事件循环": "23",
              "Python asyncio 异步并发": "23",     # 真解析器按"唯一子串"也认这个简称
              "IoT 设备接入物联网平台指南": "22",
              "Saudade Blog AI Agent（泠月喵）架构文档": "19",
              "Git从入门到入土": "16",
              "ESP32-S3-OBC固件接入参考": "14"}
    orig = ctx._doc_id_lookup
    orig_ready = ctx._corpus_ready
    ctx._doc_id_lookup = lambda t: CORPUS.get(t, "")
    ctx._corpus_ready = lambda: True
    try:
        # ① 标签 id 当文章 id（trace 20260921T232338 / 20260922T001332 的 page_ctx
        #    会话摘要，原文逐字）
        ...
        noti = _doc_anchors([HumanMessage(content=(
            "[System: · 未读通知: 26《留言未通过审核》/25《留言未通过审核》"
            "· 公告: 《中秋快乐》《今晚不许熬夜！》《致李重九》"))])
        check("锚点：通知/公告标题不进「已点名文档」清单",
              "留言未通过审核" not in noti and "中秋快乐" not in noti
              and "今晚不许熬夜" not in noti and "·" not in noti)

        # ③ 邻域不许跨过下一条标题（trace 20260920T140119 的 recent_tail，逐字：
        #    narrator 在为《IoT…》作解释时，把下一条链接写成了 /article/46）
        cross = _doc_anchors([AIMessage(content=(
        ...
        sent = _doc_anchors([AIMessage(content=(
        ...
        #    逐字）：`16《Git从入门到入土》` 后面那行 `跳转「/article/19」` 的 19
        #    属于另一次动作，不是 Git 那篇的 id
        nav = _doc_anchors([HumanMessage(content=(
        ...
        check("锚点：解析不出的标题不留占位行", "·" not in draft)

        # ⑥ 语料索引未就绪：如实注记，而不是逐条降级成"未见过 id"
...
def test_short_reply_and_adjacent_pairs():
    ...
    **本身不含意图**的短消息，要靠数行位去推断它接的是哪句提议，实测常被当成
    ...
    # —— 尾缀语气词（20260925）——
    # 同族第三例（前两例：系统消息壳、导航快道）：判据是**整串相等**型，于是任何一层
    # 外壳都能把它架空。泠月人设自带"喵"字尾，主人跟着回「要喵」「好的喵呜」是常态，
    # 旧判据把它们全判成"不是短应答" ⇒ planner 又失去那条确定性指引。
    # 锁的是**形态族**（剥句尾语气词素），不是往三个集合里加带喵写法——词表追不全。
    for t, want in (("[当前问题]: 要喵", "pos"), ("[当前问题]: 好的喵", "pos"),
                    ("[当前问题]: 嗯嗯喵", "pos"), ("要喵呜", "pos"),
                    ("[当前问题]: 不用了喵", "neg"), ("[当前问题]: 算了喵呜", "neg"),
                    ("[当前问题]: 你看着办喵", "auth"), ("[当前问题]: 都行喵", "auth")):
        check(f"尾缀语气词短应答「{t}」判 {want}",
              _short_reply_kind(t) == want, _short_reply_kind(t))
    # 不矫枉过正：只剥**句尾**的语气词，句中/别的话题一律不动；整句只剩语气词也不是短应答
    for t in ("[当前问题]: 喵", "[当前问题]: 喵喵", "[当前问题]: 我要看喵喵的贴纸",
              "[当前问题]: 把「喵呜」打开"):
        check(f"语气词不误判「{t}」", _short_reply_kind(t) == "", _short_reply_kind(t))
...
def test_write_target_refusal_round():
    ...
    try:
        ...
        # 20260926：**禁止句旁边必须给出允许的说法**。现场（trace 20260926T171235）是
        # 模型把"不许说系统的动作是我做的"反向套用成"那我只能自我否认"，对着系统上一轮
        # 写下的核对结论答「是我自己脑补的」——注记只列禁止、不给出口，等于把模型逼到
        # 另一头。同一条纪律在 prompts.py 的诚实底线里也有一份（措辞不同、语义同源）。
        check("  且点了**允许的出口**（照原样转述系统给的结论，来源说系统）",
              "照原样转述" in note and "系统" in note and "脑补" in note, note[-200:])
