...
CLASSES = [
    ...
    dict(
        ...
        why="判红的 trace 是复核红的唯一证据；「证据不足就不删」是刻意的保守规则。"
            "20260925 只读审计：58 个目录里窗口外 28 个全部留住（4 个有失败记录 + "
            "24 个旧格式留档缺 `trace_run` 连接键、对不上号）——**这是一次性存量、不是无界泄漏**",
        rule=None,
    ),
    dict(
    ...
    dict(
        ...
        why="576 份 / 9.8M，且**它是 `golden_trace.prune` 的判据源**（反查「那一晚红没红」）⇒ "
            ...
            "里面 493 份还是旧格式（有 `cases`/`failed`、没有 `trace_run` 连接键）、"
            "22 份是 recall_eval 的输出——**一个目录三个产出者**。"
