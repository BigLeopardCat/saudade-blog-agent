...
# 通知的标题/正文上限：同上，**只此一处**（tools/base.py，与 Rust `notice.rs` 的
# `TITLE_MAX` / `CONTENT_MAX` 同源）。展开层挡超长只是为了"零工具 + 说清原因"，
# 真正的硬闸在工具与服务端那两道。
from tools.base import _NOTICE_CONTENT_LIMIT, _NOTICE_TITLE_LIMIT
...
WRITE_SKILL_NAMES = frozenset({
    ...
    # 给单个账号发站内通知（20260926 第十一轮）：目标也是一个**账号名**（后台账号
    # 列表里核对得到）⇒ 同走名字通道。正文是自由文本，但它**不是**这个技能的目标
...
_WRITE_NAME_TARGET_SKILLS = frozenset({
    ...
    "account_freeze", "account_unfreeze",
    # 给单个账号发通知（20260926 第十一轮）：同一条通道（同样是
    # `tools.base._find_named_user` 对着后台账号名录解析名字）。与已存在的
    # `notice_read`（标记通知已读）**不是一对**——那一件动的是**主人自己**的通知
...
SKILLS: list[Skill] = [
    ...
    # 与 notice_read（标记通知已读）同属"通知"这一物件族、方向相反，但**不是一对**：
    # notice_read 动的是**主人自己**的未读水位（自身数据、不必弹卡），
    # notice_send 是把一段话**发到别人的个人中心**（第三方、发出去删不掉）⇒ 一律弹卡。
    # 收件人走名字通道（同冻结族，后台名录里核对得出来），正文是自由文本 ⇒
    # **刻意不进 `_WRITE_VALUE_FIELDS`**（那条闸是逐字子串判据，会把用户拍板的
    # "允许整理成一句得体的通知"结构性关死；正文的防线是卡面印全文，见 graph.py 的注）。
    # 技能名与工具名不是一套字面量（同 account_freeze/freeze_account 的惯例）。
    Skill(
        name="notice_send",
        capability="给一个后台账号发一条站内通知（会出现在对方的个人中心）",
        description=(
            "博主（管理员）要求**给某个用户/账号单独发一条通知或私信**时使用"
            "（「给〈账号名〉发个通知」「跟〈账号名〉说一声」「提醒一下〈账号名〉」"
            "——〈账号名〉是占位符，照抄主人说的那个名字）。"
            "参数 name=收件人的**账号名**（后台账号列表里看得见的那一行；"
            "**必须能在列表里看到**——列表里没有就当它不存在，**不要**用账号编号，"
            "也不要用你猜的名字）；content=通知正文；title=标题（可选，"
            "主人没说标题就**不要自己挑一个**，系统会用「站内通知」）。"
            "⚠️ 正文可以按主人的意思**整理成一句得体的话**，但**只许整理、不许添加**"
            "主人没说过的事实、承诺或威胁（「你已被警告三次」这类编造一律不许）——"
            "那段话是以**主人的名义**发给对方的。"
            "⚠️ 这条通知会出现在**对方的个人中心**、且**发出后没有撤回的通道**"
            "（站内没有删除已发通知的功能）⇒ 系统会先把收件人与正文全文打进确认卡，"
            "主人点确定才真发。"
            "⚠️ 要**全站**都能看到的那是公告，用另一个技能。**仅管理员可用**"
        ),
        inputs={
            "name": "收件人的账号名（后台账号列表里看得见的那一行）",
            "content": "通知正文（可以按主人的意思整理成一句得体的话，不许编造他没说过的事）",
            "title": "（可选）通知标题：一句话；主人没说就不填",
        },
        plan=[("send_user_notice", {"name": "$name", "content": "$content"})],
        complete_when="send_user_notice 返回了发送结果（含收件账号名）",
        reply_contract=(
            "只能按这次实际返回作答，**逐字转述后台给出的那句话**（含账号名）；"
            "返回「后台账号列表里没有叫 X 的账号」就如实说没找到、什么都没发；"
            "返回「该账号不能接收通知」就如实说这个账号收不到；"
            "被拒绝或未确认时如实说**没发出去**，**绝不得用完成式声称已送达**；"
            "**不要**替对方断言「他已经看到了」——系统只知道这条通知已进了他的通知面板，"
            "他此刻有没有打开只有他自己知道；**不要**在回复里复述技巧性细节"
            "（如账号编号、内部字段名），只说主人关心的那件事"
        ),
        roles=ADMIN_ROLES,
    ),
    Skill(
...
def _expand_write_skill(skill, params: dict) -> tuple[list[str], str]:
    ...
    **目标一律按名字**（20260921 第四轮）：planner 写「编程」而不是 id——用户嘴里
    ...
    if name == "notice_send":
        # 给单个账号发通知（20260926 第十一轮）。三件与上面两支不同的事：
        #   · **正文是必填的**（缺了零工具 + 问清要发什么——**绝不替主人编一句**：
        #     那是拿他的名义对第三方说话，收不回）；
        #   · 正文**允许整理**（用户拍板），所以这里只挡长度、不做"必须逐字"的校验
        #     ——那一层刻意不接（`graph._WRITE_VALUE_FIELDS` 的注写了为什么）；
        #   · `title` **可留空**（服务端会落一个系统默认标题），点名了才进 args。
        # 缺收件人名字那一路的措辞与冻结族同硬（名字是唯一通道，不许猜、不许用编号）。
        target = _write_arg(params.get("name"))
        if not target:
            return [], ("notice_send 缺少收件账号名（name）：不调用任何工具，"
                        "如实向主人问清这条通知要发给**哪一个账号**"
                        "——**不要**拿你猜的名字顶上，也不要用账号编号")
        if target.isdigit():
            return [], ("notice_send 给的是一串数字「" + target + "」：不调用任何工具，"
                        "如实向主人问清那个账号的**名字**（后台账号列表里看得见的那一行；"
                        "系统不支持按编号操作账号）")
        content = _write_arg(params.get("content"))
        if not content:
            return [], ("notice_send 缺少正文（content）：不调用任何工具，"
                        "如实向主人问清这条通知要说什么——**不要**替他写一句："
                        "那段话会以他的名义发给对方，且发出后没有撤回的通道")
        if len(content) > _NOTICE_CONTENT_LIMIT:
        ...
        if title and len(title) > _NOTICE_TITLE_LIMIT:
            return [], (f"notice_send 的标题太长（{len(title)} 字，上限 "
                        f"{_NOTICE_TITLE_LIMIT} 字）：不调用任何工具，"
                        "如实向主人说明太长、请他给短一点的标题")
        args = {"name": target, "content": content}
        if title:
            args["title"] = title
        return ([_spec("send_user_notice", args)],
                (f"给后台账号「{target}」发一条站内通知"
                 f"（标题「{title or '站内通知'}」）"
...
def build_planner_context(role: str | None = None) -> str:
    ...
    # 而 chat 的语义是"闲聊"⇒ "这件事我做不到"**整条链路上没有任何一层被要求说出来**，
    # narrator 便用一句承诺句式（"点「确定」我就去办"）把它盖住了。确定性出口在别的维度
    # 早就有先例（NAV_MAP 值为 None = 已下线 ⇒ 如实告知），能力的边界一直没有对应物。
    #
    # ⚠️ 上面那个例子**当天就被建出来了**（技能 notice_send / 工具 send_user_notice）
    # ⇒ 拿它当"站内没有的能力"的举例**现在是假话**，别抄回来。这一段举例只用
    # **已核实真没有**的能力（见下方"登录记录/登录 IP"那一条的依据）。
    #
    # **刻意不写进 chat 的 description**：那会让"没有对应能力"看起来像 chat 的常规用法，
    # planner 可能据此把**真有技能**的请求也判成"做不到"（代价是不动手、主人原地等着，
    # 比多问一句贵）。写在这里，并要求"拿不准就照常选技能"——失败取向往能干活那侧倒。
    lines.append(
        "\n以上技能都不覆盖主人这一轮的请求时（他要办的事站内**根本没有**对应的能力或"
        "通道——例如查某个账号的**登录记录 / 登录 IP / 登录时间**，站内只记账号的"
        "创建时间与角色，没有登录审计）："
        "选 chat，回复里**如实说做不到**，并告诉他你能做的替代是什么（例如公告是全站"
        "可见的）。**拿不准就照常选技能**：把有技能的事判成做不了，比多问一句贵得多。"
        "**禁止**用 chat 收尾去索要「要不要我办／你说一声我就去办」——不排出写操作的"
        "清单就不会有任何确认卡，那句承诺保证是假的（系统只在真排了写操作时才弹卡）。")
