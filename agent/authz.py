...
def _own_command(msg: str, tool: str | None = None) -> bool:
    ...
    if _CONSOLE_QUESTION_RE.search(text) or _CONSOLE_HYPOTHESIS_RE.search(text):
        return False
    # 打听类（「收藏文章有什么用」「取消收藏的**步骤**是什么」）：这里刻意**连着
    # `_CONSOLE_INQUIRY_RE` 一起用**（那张表本来是给弹窗分叉用的），代价是"注意/要求"
    # 这类词也会让一句真命令落回弹窗——方向的取舍与全表一致：多一次点击 < 多一次误写。
    if _CONSOLE_INQUIRY_RE.search(text) or _OWN_INQUIRY_RE.search(text):
        return False
